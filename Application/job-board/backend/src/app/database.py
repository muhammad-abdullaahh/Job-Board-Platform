...
if is_serverless:
    ...
    # For long-running servers (e.g. local uvicorn, Docker with 4 workers):
    # pool_size=3 + max_overflow=2 per worker ensures 4 workers stay within 20 total connections.
    engine = create_engine(
        DATABASE_URL,
        pool_size=3,
        max_overflow=2,
...
def ensure_db_migrated():
    """Ensure startup schema integrity once per process. Thread-safe."""
    global _db_migrated
    if not _db_migrated:
...
def get_db():
    """Request-scoped database session dependency. Strictly decoupled from DDL migrations."""
    db = SessionLocal()
    try:
        yield db
    finally:
